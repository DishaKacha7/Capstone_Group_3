# ===================== IMPORTS =====================
import pandas as pd, numpy as np, gc, joblib, os, psutil
...
joblib.dump(data['crop_name_encoded'].unique(), 'label_encoder.joblib')
...
print("XGB done.")
joblib.dump(xgb, "models/xgb_model.joblib", compress=3)
...
print("LGBM done.")
joblib.dump(lgbm, "models/lgbm_model.joblib", compress=3)
...
field_pred_df.to_csv("pixel_predictions.csv", index=False)
...
plt.tight_layout()
plt.savefig("field_confusion_matrix.png")
plt.show()
